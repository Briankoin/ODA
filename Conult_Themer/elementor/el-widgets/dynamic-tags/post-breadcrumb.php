...
class GVAElement_Post_Breadcrumb extends GVAElement_Base{
	...
	public function get_title(){
		return esc_html__('Breadcrumb', 'conult-themer');
	}
	...
	protected function register_controls(){
		...
		$this->add_control(
			'bg_overlay_color',
			[
				'label' => esc_html__( 'Background Overlay Color', 'conult-themer' ),
		...
		$this->add_control(
	  	...
	  	$this->add_responsive_control(
			'padding_top',
			[
				'label' => __( 'Breadcrumb Padding Top', 'conult-themer' ),
		...
		$this->add_responsive_control(
			'padding_bottom',
			[
				'label' => __( 'Breadcrumb Padding Bottom', 'conult-themer' ),
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => 120,
				],
		...
		$this->add_control(
			'heading_content',
			[
				'label'     => __('Content & Style ------', 'conult-themer'),
				'type'      => Controls_Manager::HEADING,
			]
	  	);
		$this->add_control(
			'show_title',
			[
				'label'     => __('Show Titlte', 'conult-themer'),
		...
		$this->add_control(
			'title_color',
			[
				'label' => esc_html__( 'Title Color', 'conult-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .custom-breadcrumb .heading-title',
			]
		);
		$this->add_control(
			'show_links',
			[
				'label'     => __('Show Breadcrumb Links', 'conult-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
	  	...
	  	$this->add_control(
			'breadcrumb_link_bg',
			[
				'label' => esc_html__( 'Background Color Breadcrumb Links', 'conult-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
	...
	public function breadcrumbs(){
	 	$delimiter = '';
	 	$home = esc_html__('Home', 'conult-themer');
	 	...
	 	if(!is_home() && !is_front_page() || is_paged()) {
			...
			if(is_category()){
			  	...
			  	$breadcrumb .= $before . esc_html__('Search results for', 'conult-themer') . '"' . get_search_query() . '"' . $after;
			  ...
			  $breadcrumb .= $before . esc_html__('Posts tagged', 'conult-themer') . '"' . single_tag_title('', false) . '"' . $after;
			}elseif(is_author()){
			  	global $author;
			  	$userdata = get_userdata($author);
			  	if($userdata){
				 	$breadcrumb .= $before . esc_html__('Articles posted by', 'conult-themer') . $userdata->display_name . $after;
			  	} 
			}elseif(is_404()){
			  	$breadcrumb .= $before . esc_html__('Error 404', 'conult-themer') . $after;
			}
			if(get_query_var('paged')){
			  	$breadcrumb .= $before . esc_html__('Page','conult-themer') . ' ' . get_query_var('paged') . $after;
			}
			$breadcrumb .= '</ol>';
