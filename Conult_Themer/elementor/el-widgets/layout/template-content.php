...
class GVAElement_Template_Content extends  GVAElement_Base {
	...
	public function get_title(){
		return __('GVA Tempate Content', 'conult-themer');
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_content',
			[
				'label' => __('General', 'conult-themer'),
			]
	  	);
		$this->add_control(
			'title_text',
		...
		if($post_type === 'gva__template'){
			...
			if($template_type == 'page_layout'){
				echo '<div style="padding: 300px 30px 250px;background: #525252;">';
			 		echo '<h1 style="color: #fff;text-align: center;">' . esc_html__('Your page content will be displayed here', 'conult-themer') . '</h1>';
			 	echo '</div>';
			}else{
			 	the_content();
			}
