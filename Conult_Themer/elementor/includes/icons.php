<?php
   add_filter( 'elementor/icons_manager/additional_tabs', 'conult_custom_icons_filters' , 9999999, 1 );
   function conult_custom_icons_filters( $tabs = array() ) {
      $newicons = [
         "flaticon-email",
         "flaticon-email-1",
         "flaticon-pin",
         "flaticon-pin-1",
         "flaticon-networking",
         "flaticon-photograph",
         "flaticon-translation",
         "flaticon-stethoscope",
         "flaticon-skincare",
         "flaticon-computer",
         "flaticon-graphic-designer",
         "flaticon-checkmark",
         "flaticon-scrum",
         "flaticon-mission",
         "flaticon-mission-1",
         "flaticon-team",
         "flaticon-team-1",
         "flaticon-increment",
         "flaticon-increment-1",
         "flaticon-left-arrow",
         "flaticon-handshake",
         "flaticon-thumbs-up",
         "flaticon-thumbs-up-1",
         "flaticon-creativity",
         "flaticon-creativity-1",
         "flaticon-invention",
         "flaticon-smartphone",
         "flaticon-smartphone-1",
         "flaticon-nanotechnology",
         "flaticon-nanotechnology-1",
         "flaticon-browser",
         "flaticon-web",
         "flaticon-crowdfunding",
         "flaticon-crowdfunding-1",
         "flaticon-crowdfunding-2",
         "flaticon-crowdfunding-3",
         "flaticon-fundraise",
         "flaticon-crowdfunding-4",
         "flaticon-chat",
         "flaticon-speech-bubble",
         "flaticon-chat-1",
         "flaticon-plus",
         "flaticon-minus",
         "flaticon-add",
         "flaticon-search",
         "flaticon-search-1",
         "flaticon-call",
         "flaticon-email-2",
         "flaticon-location",
         "flaticon-entrepreneur",
         "flaticon-location-1",
         "flaticon-contact",
         "flaticon-pin-2",
         "flaticon-check",
         "flaticon-checked", 
         "flaticon-share",
         "icon-conult-pin",
         "icon-conult-email",
         "icon-conult-phone",
         "icon-conult-magnifying-glass",
         "icon-conult-shopping-cart",
         "icon-conult-right-arrow",
         "icon-conult-strategy",
         "icon-conult-point-of-sale",
         "icon-conult-consumer-behavior",
         "icon-conult-verification",
         "icon-conult-quotes",
         "icon-conult-help",
         "icon-conult-customer-review",
         "icon-conult-consultant",
         "icon-conult-telephone",
         "icon-conult-business",
         "icon-conult-creative",
         "icon-conult-global",
         "icon-conult-news",
         "icon-conult-consulting",
         "icon-conult-group",
         "icon-conult-report",
         "icon-conult-mobile-analytics",
         "icon-conult-creative-1",
         "icon-conult-analysis"
      ];

      $icons['gva_custom_icon'] = array(
           'name'          => 'conult-icons-theme',
           'label'         => esc_html__( 'Icons Theme', 'conult-themer' ),
           'labelIcon'     => 'fas fa-user',
           'prefix'        => '',
           'displayPrefix' => '',
           'icons'         => $newicons,
           'url'          => GAVIAS_CONULT_PLUGIN_URL . 'assets/icons/style.css',
           'ver'           => '1.0',
       );

      return array_merge( $tabs, $icons );
   }

   add_action( 'wp_print_footer_scripts', 'conult_insert_icons_footer_css'  );
   function conult_insert_icons_footer_css() {
      echo '<link rel="stylesheet" type="text/css" href="' . GAVIAS_CONULT_PLUGIN_URL . 'assets/icons/style.css">';
   }

   add_filter( 'elementor/icons_manager/additional_tabs', 'conult_lineawesome_icon' );  
   function conult_lineawesome_icon( $icons = array() ) {
       $lineawesome_icons = array(
         'lab la-500px',
         'lab la-accessible-icon',
         'lab la-accusoft',
         'lab la-acquisitions-incorporated',
         'las la-ad',
         'las la-address-book',
         'las la-address-card',
         'las la-adjust',
         'lab la-adn',
         'lab la-adobe',
         'lab la-adversal',
         'lab la-affiliatetheme',
         'las la-air-freshener',
         'lab la-airbnb',
         'lab la-algolia',
         'las la-align-center',
         'las la-align-justify',
         'las la-align-left',
         'las la-align-right',
         'lab la-alipay',
         'la la-allergies',
         'lab la-amazon',
         'lab la-amazon-pay',
         'las la-ambulance',
         'las la-american-sign-language-interpreting',
         'lab la-amilia',
         'las la-anchor',
         'lab la-android',
         'lab la-angellist',
         'las la-angle-double-down',
         'las la-angle-double-left',
         'las la-angle-double-right',
         'las la-angle-double-up',
         'las la-angle-down',
         'las la-angle-left',
         'las la-angle-right',
         'las la-angle-up',
         'las la-angry',
         'lab la-angrycreative',
         'lab la-angular',
         'las la-ankh',
         'lab la-app-store',
         'lab la-app-store-ios',
         'lab la-apper',
         'lab la-apple',
         'las la-apple-alt',
         'lab la-apple-pay',
         'las la-archive',
         'las la-archway',
         'las la-arrow-alt-circle-down',
         'las la-arrow-alt-circle-left',
         'las la-arrow-alt-circle-right',
         'las la-arrow-alt-circle-up',
         'las la-arrow-circle-down',
         'las la-arrow-circle-left',
         'las la-arrow-circle-right',
         'las la-arrow-circle-up',
         'las la-arrow-down',
         'las la-arrow-left',
         'las la-arrow-right',
         'las la-arrow-up',
         'las la-arrows-alt',
         'las la-arrows-alt-h',
         'las la-arrows-alt-v',
         'lab la-artstation',
         'las la-assistive-listening-systems',
         'las la-asterisk',
         'lab la-asymmetrik',
         'las la-at',
         'las la-atlas',
         'lab la-atlassian',
         'las la-atom',
         'lab la-audible',
         'las la-audio-description',
         'lab la-autoprefixer',
         'lab la-avianex',
         'lab la-aviato',
         'las la-award',
         'lab la-aws',
         'la la-baby',
         'las la-baby-carriage',
         'las la-backspace',
         'las la-backward',
         'las la-bacon',
         'las la-balance-scale',
         'las la-balance-scale-left',
         'las la-balance-scale-right',
         'las la-ban',
         'las la-band-aid',
         'lab la-bandcamp',
         'las la-barcode',
         'las la-bars',
         'las la-baseball-ball',
         'las la-basketball-ball',
         'las la-bath',
         'las la-battery-empty',
         'las la-battery-full',
         'las la-battery-half',
         'las la-battery-quarter',
         'las la-battery-three-quarters',
         'lab la-battle-net',
         'las la-bed',
         'las la-beer',
         'lab la-behance',
         'lab la-behance-square',
         'las la-bell',
         'las la-bell-slash',
         'las la-bezier-curve',
         'las la-bible',
         'las la-bicycle',
         'las la-biking',
         'lab la-bimobject',
         'las la-binoculars',
         'las la-biohazard',
         'las la-birthday-cake',
         'lab la-bitbucket',
         'lab la-bitcoin',
         'lab la-bity',
         'lab la-black-tie',
         'lab la-blackberry',
         'las la-blender',
         'las la-blender-phone',
         'las la-blind',
         'las la-blog',
         'lab la-blogger',
         'lab la-blogger-b',
         'lab la-bluetooth',
         'lab la-bluetooth-b',
         'las la-bold',
         'las la-bolt',
         'las la-bomb',
         'las la-bone',
         'las la-bong',
         'las la-book',
         'las la-book-dead',
         'las la-book-medical',
         'las la-book-open',
         'las la-book-reader',
         'las la-bookmark',
         'lab la-bootstrap',
         'las la-border-all',
         'las la-border-none',
         'las la-border-style',
         'las la-bowling-ball',
         'las la-box',
         'las la-box-open',
         'las la-boxes',
         'las la-braille',
         'las la-brain',
         'las la-bread-slice',
         'las la-briefcase',
         'las la-briefcase-medical',
         'las la-broadcast-tower',
         'las la-broom',
         'las la-brush',
         'lab la-btc',
         'lab la-buffer',
         'las la-bug',
         'las la-building',
         'las la-bullhorn',
         'las la-bullseye',
         'las la-burn',
         'lab la-buromobelexperte',
         'las la-bus',
         'las la-bus-alt',
         'las la-business-time',
         'lab la-buy-n-large',
         'lab la-buysellads',
         'las la-calculator',
         'las la-calendar',
         'las la-calendar-alt',
         'las la-calendar-check',
         'las la-calendar-day',
         'las la-calendar-minus',
         'las la-calendar-plus',
         'las la-calendar-times',
         'las la-calendar-week',
         'las la-camera',
         'las la-camera-retro',
         'las la-campground',
         'lab la-canadian-maple-leaf',
         'las la-candy-cane',
         'las la-cannabis',
         'las la-capsules',
         'las la-car',
         'las la-car-alt',
         'las la-car-battery',
         'las la-car-crash',
         'las la-car-side',
         'las la-caret-down',
         'las la-caret-left',
         'las la-caret-right',
         'las la-caret-square-down',
         'las la-caret-square-left',
         'las la-caret-square-right',
         'las la-caret-square-up',
         'las la-caret-up',
         'las la-carrot',
         'las la-cart-arrow-down',
         'las la-cart-plus',
         'las la-cash-register',
         'las la-cat',
         'lab la-cc-amazon-pay',
         'lab la-cc-amex',
         'lab la-cc-apple-pay',
         'lab la-cc-diners-club',
         'lab la-cc-discover',
         'lab la-cc-jcb',
         'lab la-cc-mastercard',
         'lab la-cc-paypal',
         'lab la-cc-stripe',
         'lab la-cc-visa',
         'lab la-centercode',
         'lab la-centos',
         'las la-certificate',
         'las la-chair',
         'las la-chalkboard',
         'las la-chalkboard-teacher',
         'las la-charging-station',
         'las la-chart-area',
         'las la-chart-bar',
         'las la-chart-line',
         'las la-chart-pie',
         'las la-check',
         'las la-check-circle',
         'las la-check-double',
         'las la-check-square',
         'las la-cheese',
         'las la-chess',
         'las la-chess-bishop',
         'las la-chess-board',
         'las la-chess-king',
         'las la-chess-knight',
         'las la-chess-pawn',
         'las la-chess-queen',
         'las la-chess-rook',
         'las la-chevron-circle-down',
         'las la-chevron-circle-left',
         'las la-chevron-circle-right',
         'las la-chevron-circle-up',
         'las la-chevron-down',
         'las la-chevron-left',
         'las la-chevron-right',
         'las la-chevron-up',
         'las la-child',
         'lab la-chrome',
         'lab la-chromecast',
         'las la-church',
         'las la-circle',
         'las la-circle-notch',
         'las la-city',
         'las la-clinic-medical',
         'las la-clipboard',
         'las la-clipboard-check',
         'las la-clipboard-list',
         'las la-clock',
         'las la-clone',
         'las la-closed-captioning',
         'las la-cloud',
         'las la-cloud-download-alt',
         'las la-cloud-meatball',
         'las la-cloud-moon',
         'las la-cloud-moon-rain',
         'las la-cloud-rain',
         'las la-cloud-showers-heavy',
         'las la-cloud-sun',
         'las la-cloud-sun-rain',
         'las la-cloud-upload-alt',
         'lab la-cloudscale',
         'lab la-cloudsmith',
         'lab la-cloudversify',
         'las la-cocktail',
         'las la-code',
         'las la-code-branch',
         'lab la-codepen',
         'lab la-codiepie',
         'las la-coffee',
         'las la-cog',
         'las la-cogs',
         'las la-coins',
         'las la-columns',
         'las la-comment',
         'las la-comment-alt',
         'las la-comment-dollar',
         'las la-comment-dots',
         'las la-comment-medical',
         'las la-comment-slash',
         'las la-comments',
         'las la-comments-dollar',
         'las la-compact-disc',
         'las la-compass',
         'las la-compress',
         'las la-compress-arrows-alt',
         'las la-concierge-bell',
         'lab la-confluence',
         'lab la-connectdevelop',
         'lab la-contao',
         'las la-cookie',
         'las la-cookie-bite',
         'las la-copy',
         'las la-copyright',
         'lab la-cotton-bureau',
         'las la-couch',
         'lab la-cpanel',
         'lab la-creative-commons',
         'lab la-creative-commons-by',
         'lab la-creative-commons-nc',
         'lab la-creative-commons-nc-eu',
         'lab la-creative-commons-nc-jp',
         'lab la-creative-commons-nd',
         'lab la-creative-commons-pd',
         'lab la-creative-commons-pd-alt',
         'lab la-creative-commons-remix',
         'lab la-creative-commons-sa',
         'lab la-creative-commons-sampling',
         'lab la-creative-commons-sampling-plus',
         'lab la-creative-commons-share',
         'lab la-creative-commons-zero',
         'las la-credit-card',
         'lab la-critical-role',
         'las la-crop',
         'las la-crop-alt',
         'las la-cross',
         'las la-crosshairs',
         'las la-crow',
         'las la-crown',
         'las la-crutch',
         'lab la-css3',
         'lab la-css3-alt',
         'las la-cube',
         'las la-cubes',
         'las la-cut',
         'lab la-cuttlefish',
         'lab la-d-and-d',
         'lab la-d-and-d-beyond',
         'lab la-dashcube',
         'las la-database',
         'las la-deaf',
         'lab la-delicious',
         'las la-democrat',
         'lab la-deploydog',
         'lab la-deskpro',
         'las la-desktop',
         'lab la-dev',
         'lab la-deviantart',
         'las la-dharmachakra',
         'lab la-dhl',
         'las la-diagnoses',
         'lab la-diaspora',
         'las la-dice',
         'las la-dice-d20',
         'las la-dice-d6',
         'las la-dice-five',
         'las la-dice-four',
         'las la-dice-one',
         'las la-dice-six',
         'las la-dice-three',
         'las la-dice-two',
         'lab la-digg',
         'lab la-digital-ocean',
         'las la-digital-tachograph',
         'las la-directions',
         'lab la-discord',
         'lab la-discourse',
         'las la-divide',
         'las la-dizzy',
         'las la-dna',
         'lab la-dochub',
         'lab la-docker',
         'las la-dog',
         'las la-dollar-sign',
         'las la-dolly',
         'las la-dolly-flatbed',
         'las la-donate',
         'las la-door-closed',
         'las la-door-open',
         'las la-dot-circle',
         'las la-dove',
         'las la-download',
         'lab la-draft2digital',
         'las la-drafting-compass',
         'las la-dragon',
         'las la-draw-polygon',
         'lab la-dribbble',
         'lab la-dribbble-square',
         'lab la-dropbox',
         'las la-drum',
         'las la-drum-steelpan',
         'las la-drumstick-bite',
         'lab la-drupal',
         'las la-dumbbell',
         'las la-dumpster',
         'las la-dumpster-fire',
         'las la-dungeon',
         'lab la-dyalog',
         'lab la-earlybirds',
         'lab la-ebay',
         'lab la-edge',
         'las la-edit',
         'las la-egg',
         'las la-eject',
         'las la-elementor',
         'las la-ellipsis-h',
         'las la-ellipsis-v',
         'lab la-ello',
         'lab la-ember',
         'lab la-empire',
         'las la-envelope',
         'las la-envelope-open',
         'las la-envelope-open-text',
         'las la-envelope-square',
         'lab la-envira',
         'las la-equals',
         'las la-eraser',
         'lab la-erlang',
         'lab la-ethereum',
         'las la-ethernet',
         'lab la-etsy',
         'las la-euro-sign',
         'lab la-evernote',
         'las la-exchange-alt',
         'las la-exclamation',
         'las la-exclamation-circle',
         'las la-exclamation-triangle',
         'las la-expand',
         'las la-expand-arrows-alt',
         'lab la-expeditedssl',
         'las la-external-link-alt',
         'las la-external-link-square-alt',
         'las la-eye',
         'las la-eye-dropper',
         'las la-eye-slash',
         'lab la-facebook',
         'lab la-facebook-f',
         'lab la-facebook-messenger',
         'lab la-facebook-square',
         'las la-fan',
         'lab la-fantasy-flight-games',
         'las la-fast-backward',
         'las la-fast-forward',
         'las la-fax',
         'las la-feather',
         'las la-feather-alt',
         'lab la-fedex',
         'lab la-fedora',
         'las la-female',
         'las la-fighter-jet',
         'lab la-figma',
         'las la-file',
         'las la-file-alt',
         'las la-file-archive',
         'las la-file-audio',
         'las la-file-code',
         'las la-file-contract',
         'las la-file-csv',
         'las la-file-download',
         'las la-file-excel',
         'las la-file-export',
         'las la-file-image',
         'las la-file-import',
         'las la-file-invoice',
         'las la-file-invoice-dollar',
         'las la-file-medical',
         'las la-file-medical-alt',
         'las la-file-pdf',
         'las la-file-powerpoint',
         'las la-file-prescription',
         'las la-file-signature',
         'las la-file-upload',
         'las la-file-video',
         'las la-file-word',
         'las la-fill',
         'las la-fill-drip',
         'las la-film',
         'las la-filter',
         'las la-fingerprint',
         'las la-fire',
         'las la-fire-alt',
         'las la-fire-extinguisher',
         'lab la-firefox',
         'las la-first-aid',
         'lab la-first-order',
         'lab la-first-order-alt',
         'lab la-firstdraft',
         'las la-fish',
         'las la-fist-raised',
         'las la-flag',
         'las la-flag-checkered',
         'las la-flag-usa',
         'las la-flask',
         'lab la-flickr',
         'lab la-flipboard',
         'las la-flushed',
         'lab la-fly',
         'las la-folder',
         'las la-folder-minus',
         'las la-folder-open',
         'las la-folder-plus',
         'las la-font',
         'lab la-font-awesome',
         'lab la-font-awesome-alt',
         'lab la-font-awesome-flag',
         'lab la-fonticons',
         'lab la-fonticons-fi',
         'las la-football-ball',
         'lab la-fort-awesome',
         'lab la-fort-awesome-alt',
         'lab la-forumbee',
         'las la-forward',
         'lab la-foursquare',
         'lab la-free-code-camp',
         'lab la-freebsd',
         'las la-frog',
         'las la-frown',
         'las la-frown-open',
         'lab la-fulcrum',
         'las la-funnel-dollar',
         'las la-futbol',
         'lab la-galactic-republic',
         'lab la-galactic-senate',
         'las la-gamepad',
         'las la-gas-pump',
         'las la-gavel',
         'las la-gem',
         'las la-genderless',
         'lab la-get-pocket',
         'lab la-gg',
         'lab la-gg-circle',
         'las la-ghost',
         'las la-gift',
         'las la-gifts',
         'lab la-git',
         'lab la-git-alt',
         'lab la-git-square',
         'lab la-github',
         'lab la-github-alt',
         'lab la-github-square',
         'lab la-gitkraken',
         'lab la-gitlab',
         'lab la-gitter',
         'las la-glass-cheers',
         'las la-glass-martini',
         'las la-glass-martini-alt',
         'las la-glass-whiskey',
         'las la-glasses',
         'lab la-glide',
         'lab la-glide-g',
         'las la-globe',
         'las la-globe-africa',
         'las la-globe-americas',
         'las la-globe-asia',
         'las la-globe-europe',
         'lab la-gofore',
         'las la-golf-ball',
         'lab la-goodreads',
         'lab la-goodreads-g',
         'lab la-google',
         'lab la-google-drive',
         'lab la-google-play',
         'lab la-google-plus',
         'lab la-google-plus-g',
         'lab la-google-plus-square',
         'lab la-google-wallet',
         'las la-gopuram',
         'las la-graduation-cap',
         'lab la-gratipay',
         'lab la-grav',
         'las la-greater-than',
         'las la-greater-than-equal',
         'las la-grimace',
         'las la-grin',
         'las la-grin-alt',
         'las la-grin-beam',
         'las la-grin-beam-sweat',
         'las la-grin-hearts',
         'las la-grin-squint',
         'las la-grin-squint-tears',
         'las la-grin-stars',
         'las la-grin-tears',
         'las la-grin-tongue',
         'las la-grin-tongue-squint',
         'las la-grin-tongue-wink',
         'las la-grin-wink',
         'las la-grip-horizontal',
         'las la-grip-lines',
         'las la-grip-lines-vertical',
         'las la-grip-vertical',
         'lab la-gripfire',
         'lab la-grunt',
         'las la-guitar',
         'lab la-gulp',
         'las la-h-square',
         'lab la-hacker-news',
         'lab la-hacker-news-square',
         'lab la-hackerrank',
         'las la-hamburger',
         'las la-hammer',
         'las la-hamsa',
         'las la-hand-holding',
         'las la-hand-holding-heart',
         'las la-hand-holding-usd',
         'las la-hand-lizard',
         'las la-hand-middle-finger',
         'las la-hand-paper',
         'las la-hand-peace',
         'las la-hand-point-down',
         'las la-hand-point-left',
         'las la-hand-point-right',
         'las la-hand-point-up',
         'las la-hand-pointer',
         'las la-hand-rock',
         'las la-hand-scissors',
         'las la-hand-spock',
         'las la-hands',
         'las la-hands-helping',
         'las la-handshake',
         'las la-hanukiah',
         'las la-hard-hat',
         'las la-hashtag',
         'las la-hat-cowboy',
         'las la-hat-cowboy-side',
         'las la-hat-wizard',
         'las la-haykal',
         'las la-hdd',
         'las la-heading',
         'las la-headphones',
         'las la-headphones-alt',
         'las la-headset',
         'las la-heart',
         'las la-heart-broken',
         'las la-heartbeat',
         'las la-helicopter',
         'las la-highlighter',
         'las la-hiking',
         'las la-hippo',
         'lab la-hips',
         'lab la-hire-a-helper',
         'las la-history',
         'las la-hockey-puck',
         'las la-holly-berry',
         'las la-home',
         'lab la-hooli',
         'lab la-hornbill',
         'las la-horse',
         'las la-horse-head',
         'las la-hospital',
         'las la-hospital-alt',
         'las la-hospital-symbol',
         'las la-hot-tub',
         'las la-hotdog',
         'las la-hotel',
         'lab la-hotjar',
         'las la-hourglass',
         'las la-hourglass-end',
         'las la-hourglass-half',
         'las la-hourglass-start',
         'las la-house-damage',
         'lab la-houzz',
         'las la-hryvnia',
         'lab la-html5',
         'lab la-hubspot',
         'las la-i-cursor',
         'las la-ice-cream',
         'las la-icicles',
         'las la-icons',
         'las la-id-badge',
         'las la-id-card',
         'las la-id-card-alt',
         'las la-igloo',
         'las la-image',
         'las la-images',
         'lab la-imdb',
         'las la-inbox',
         'las la-indent',
         'las la-industry',
         'las la-infinity',
         'las la-info',
         'las la-info-circle',
         'lab la-instagram',
         'lab la-intercom',
         'lab la-internet-explorer',
         'lab la-invision',
         'lab la-ioxhost',
         'las la-italic',
         'lab la-itch-io',
         'lab la-itunes',
         'lab la-itunes-note',
         'lab la-java',
         'las la-jedi',
         'lab la-jedi-order',
         'lab la-jenkins',
         'lab la-jira',
         'lab la-joget',
         'las la-joint',
         'lab la-joomla',
         'las la-journal-whills',
         'lab la-js',
         'lab la-js-square',
         'lab la-jsfiddle',
         'las la-kaaba',
         'lab la-kaggle',
         'las la-key',
         'lab la-keybase',
         'las la-keyboard',
         'lab la-keycdn',
         'las la-khanda',
         'lab la-kickstarter',
         'lab la-kickstarter-k',
         'las la-kiss',
         'las la-kiss-beam',
         'las la-kiss-wink-heart',
         'las la-kiwi-bird',
         'lab la-korvue',
         'las la-landmark',
         'las la-language',
         'las la-laptop',
         'las la-laptop-code',
         'las la-laptop-medical',
         'lab la-laravel',
         'lab la-lastfm',
         'lab la-lastfm-square',
         'las la-laugh',
         'las la-laugh-beam',
         'las la-laugh-squint',
         'las la-laugh-wink',
         'las la-layer-group',
         'las la-leaf',
         'lab la-leanpub',
         'las la-lemon',
         'lab la-less',
         'las la-less-than',
         'las la-less-than-equal',
         'las la-level-down-alt',
         'las la-level-up-alt',
         'las la-life-ring',
         'las la-lightbulb',
         'lab la-line',
         'las la-link',
         'lab la-linkedin',
         'lab la-linkedin-in',
         'lab la-linode',
         'lab la-linux',
         'las la-lira-sign',
         'las la-list',
         'las la-list-alt',
         'las la-list-ol',
         'las la-list-ul',
         'las la-location-arrow',
         'las la-lock',
         'las la-lock-open',
         'las la-long-arrow-alt-down',
         'las la-long-arrow-alt-left',
         'las la-long-arrow-alt-right',
         'las la-long-arrow-alt-up',
         'las la-low-vision',
         'las la-luggage-cart',
         'lab la-lyft',
         'lab la-magento',
         'las la-magic',
         'las la-magnet',
         'las la-mail-bulk',
         'lab la-mailchimp',
         'las la-male',
         'lab la-mandalorian',
         'las la-map',
         'las la-map-marked',
         'las la-map-marked-alt',
         'las la-map-marker',
         'las la-map-marker-alt',
         'las la-map-pin',
         'las la-map-signs',
         'lab la-markdown',
         'las la-marker',
         'las la-mars',
         'las la-mars-double',
         'las la-mars-stroke',
         'las la-mars-stroke-h',
         'las la-mars-stroke-v',
         'las la-mask',
         'lab la-mastodon',
         'lab la-maxcdn',
         'lab la-mdb',
         'las la-medal',
         'lab la-medapps',
         'lab la-medium',
         'lab la-medium-m',
         'las la-medkit',
         'lab la-medrt',
         'lab la-meetup',
         'lab la-megaport',
         'las la-meh',
         'las la-meh-blank',
         'las la-meh-rolling-eyes',
         'las la-memory',
         'lab la-mendeley',
         'las la-menorah',
         'las la-mercury',
         'las la-meteor',
         'las la-microchip',
         'las la-microphone',
         'las la-microphone-alt',
         'las la-microphone-alt-slash',
         'las la-microphone-slash',
         'las la-microscope',
         'lab la-microsoft',
         'las la-minus',
         'las la-minus-circle',
         'las la-minus-square',
         'las la-mitten',
         'lab la-mix',
         'lab la-mixcloud',
         'lab la-mizuni',
         'las la-mobile',
         'las la-mobile-alt',
         'lab la-modx',
         'lab la-monero',
         'las la-money-bill',
         'las la-money-bill-alt',
         'las la-money-bill-wave',
         'las la-money-bill-wave-alt',
         'las la-money-check',
         'las la-money-check-alt',
         'las la-monument',
         'las la-moon',
         'las la-mortar-pestle',
         'las la-mosque',
         'las la-motorcycle',
         'las la-mountain',
         'las la-mouse',
         'las la-mouse-pointer',
         'las la-mug-hot',
         'las la-music',
         'lab la-napster',
         'lab la-neos',
         'las la-network-wired',
         'las la-neuter',
         'las la-newspaper',
         'lab la-nimblr',
         'lab la-node',
         'lab la-node-js',
         'las la-not-equal',
         'las la-notes-medical',
         'lab la-npm',
         'lab la-ns8',
         'lab la-nutritionix',
         'las la-object-group',
         'las la-object-ungroup',
         'lab la-odnoklassniki',
         'lab la-odnoklassniki-square',
         'las la-oil-can',
         'lab la-old-republic',
         'las la-om',
         'lab la-opencart',
         'lab la-openid',
         'lab la-opera',
         'lab la-optin-monster',
         'lab la-orcid',
         'lab la-osi',
         'las la-otter',
         'las la-outdent',
         'lab la-page4',
         'lab la-pagelines',
         'las la-pager',
         'las la-paint-brush',
         'las la-paint-roller',
         'las la-palette',
         'lab la-palfed',
         'las la-pallet',
         'las la-paper-plane',
         'las la-paperclip',
         'las la-parachute-box',
         'las la-paragraph',
         'las la-parking',
         'las la-passport',
         'las la-pastafarianism',
         'las la-paste',
         'lab la-patreon',
         'las la-pause',
         'las la-pause-circle',
         'las la-paw',
         'lab la-paypal',
         'las la-peace',
         'las la-pen',
         'las la-pen-alt',
         'las la-pen-fancy',
         'las la-pen-nib',
         'las la-pen-square',
         'las la-pencil-alt',
         'las la-pencil-ruler',
         'lab la-penny-arcade',
         'las la-people-carry',
         'las la-pepper-hot',
         'las la-percent',
         'las la-percentage',
         'lab la-periscope',
         'las la-person-booth',
         'lab la-phabricator',
         'lab la-phoenix-framework',
         'lab la-phoenix-squadron',
         'las la-phone',
         'las la-phone-alt',
         'las la-phone-slash',
         'las la-phone-square',
         'las la-phone-square-alt',
         'las la-phone-volume',
         'las la-photo-video',
         'lab la-php',
         'lab la-pied-piper',
         'lab la-pied-piper-alt',
         'lab la-pied-piper-hat',
         'lab la-pied-piper-pp',
         'las la-piggy-bank',
         'las la-pills',
         'lab la-pinterest',
         'lab la-pinterest-p',
         'lab la-pinterest-square',
         'las la-pizza-slice',
         'las la-place-of-worship',
         'las la-plane',
         'las la-plane-arrival',
         'las la-plane-departure',
         'las la-play',
         'las la-play-circle',
         'lab la-playstation',
         'las la-plug',
         'las la-plus',
         'las la-plus-circle',
         'las la-plus-square',
         'las la-podcast',
         'las la-poll',
         'las la-poll-h',
         'las la-poo',
         'las la-poo-storm',
         'las la-poop',
         'las la-portrait',
         'las la-pound-sign',
         'las la-power-off',
         'las la-pray',
         'las la-praying-hands',
         'las la-prescription',
         'las la-prescription-bottle',
         'las la-prescription-bottle-alt',
         'las la-print',
         'las la-procedures',
         'lab la-product-hunt',
         'las la-project-diagram',
         'lab la-pushed',
         'las la-puzzle-piece',
         'lab la-python',
         'lab la-qq',
         'las la-qrcode',
         'las la-question',
         'las la-question-circle',
         'las la-quidditch',
         'lab la-quinscape',
         'lab la-quora',
         'las la-quote-left',
         'las la-quote-right',
         'las la-quran',
         'lab la-r-project',
         'las la-radiation',
         'las la-radiation-alt',
         'las la-rainbow',
         'las la-random',
         'lab la-raspberry-pi',
         'lab la-ravelry',
         'lab la-react',
         'lab la-reacteurope',
         'lab la-readme',
         'lab la-rebel',
         'las la-receipt',
         'las la-record-vinyl',
         'las la-recycle',
         'lab la-red-river',
         'lab la-reddit',
         'lab la-reddit-alien',
         'lab la-reddit-square',
         'lab la-redhat',
         'la la-redo',
         'las la-redo-alt',
         'las la-registered',
         'las la-remove-format',
         'lab la-renren',
         'las la-reply',
         'las la-reply-all',
         'lab la-replyd',
         'las la-republican',
         'lab la-researchgate',
         'lab la-resolving',
         'las la-restroom',
         'las la-retweet',
         'lab la-rev',
         'las la-ribbon',
         'las la-ring',
         'las la-road',
         'las la-robot',
         'las la-rocket',
         'lab la-rocketchat',
         'lab la-rockrms',
         'las la-route',
         'las la-rss',
         'las la-rss-square',
         'las la-ruble-sign',
         'las la-ruler',
         'las la-ruler-combined',
         'las la-ruler-horizontal',
         'las la-ruler-vertical',
         'las la-running',
         'las la-rupee-sign',
         'las la-sad-cry',
         'las la-sad-tear',
         'lab la-safari',
         'lab la-salesforce',
         'lab la-sass',
         'las la-satellite',
         'las la-satellite-dish',
         'las la-save',
         'lab la-schlix',
         'las la-school',
         'las la-screwdriver',
         'lab la-scribd',
         'las la-scroll',
         'las la-sd-card',
         'las la-search',
         'las la-search-dollar',
         'las la-search-location',
         'las la-search-minus',
         'las la-search-plus',
         'lab la-searchengin',
         'las la-seedling',
         'lab la-sellcast',
         'lab la-sellsy',
         'las la-server',
         'lab la-servicestack',
         'las la-shapes',
         'las la-share',
         'las la-share-alt',
         'las la-share-alt-square',
         'las la-share-square',
         'las la-shekel-sign',
         'las la-shield-alt',
         'las la-ship',
         'las la-shipping-fast',
         'lab la-shirtsinbulk',
         'las la-shoe-prints',
         'las la-shopping-bag',
         'las la-shopping-basket',
         'las la-shopping-cart',
         'lab la-shopware',
         'las la-shower',
         'las la-shuttle-van',
         'las la-sign',
         'las la-sign-in-alt',
         'las la-sign-language',
         'las la-sign-out-alt',
         'las la-signal',
         'las la-signature',
         'las la-sim-card',
         'lab la-simplybuilt',
         'lab la-sistrix',
         'las la-sitemap',
         'lab la-sith',
         'las la-skating',
         'lab la-sketch',
         'las la-skiing',
         'las la-skiing-nordic',
         'las la-skull',
         'las la-skull-crossbones',
         'lab la-skyatlas',
         'lab la-skype',
         'lab la-slack',
         'lab la-slack-hash',
         'las la-slash',
         'las la-sleigh',
         'las la-sliders-h',
         'lab la-slideshare',
         'las la-smile',
         'las la-smile-beam',
         'las la-smile-wink',
         'las la-smog',
         'las la-smoking',
         'las la-smoking-ban',
         'las la-sms',
         'lab la-snapchat',
         'lab la-snapchat-ghost',
         'lab la-snapchat-square',
         'las la-snowboarding',
         'las la-snowflake',
         'las la-snowman',
         'las la-snowplow',
         'las la-socks',
         'las la-solar-panel',
         'las la-sort',
         'las la-sort-alpha-down',
         'las la-sort-alpha-down-alt',
         'las la-sort-alpha-up',
         'las la-sort-alpha-up-alt',
         'las la-sort-amount-down',
         'las la-sort-amount-down-alt',
         'las la-sort-amount-up',
         'las la-sort-amount-up-alt',
         'las la-sort-down',
         'las la-sort-numeric-down',
         'las la-sort-numeric-down-alt',
         'las la-sort-numeric-up',
         'las la-sort-numeric-up-alt',
         'las la-sort-up',
         'lab la-soundcloud',
         'lab la-sourcetree',
         'las la-spa',
         'las la-space-shuttle',
         'lab la-speakap',
         'lab la-speaker-deck',
         'las la-spell-check',
         'las la-spider',
         'las la-spinner',
         'las la-splotch',
         'lab la-spotify',
         'las la-spray-can',
         'las la-square',
         'las la-square-full',
         'las la-square-root-alt',
         'lab la-squarespace',
         'lab la-stack-exchange',
         'lab la-stack-overflow',
         'lab la-stackpath',
         'las la-stamp',
         'las la-star',
         'las la-star-and-crescent',
         'las la-star-half',
         'las la-star-half-alt',
         'las la-star-of-david',
         'las la-star-of-life',
         'lab la-staylinked',
         'lab la-steam',
         'lab la-steam-square',
         'lab la-steam-symbol',
         'las la-step-backward',
         'las la-step-forward',
         'las la-stethoscope',
         'lab la-sticker-mule',
         'las la-sticky-note',
         'las la-stop',
         'las la-stop-circle',
         'las la-stopwatch',
         'las la-store',
         'las la-store-alt',
         'lab la-strava',
         'las la-stream',
         'las la-street-view',
         'las la-strikethrough',
         'lab la-stripe',
         'lab la-stripe-s',
         'las la-stroopwafel',
         'lab la-studiovinari',
         'lab la-stumbleupon',
         'lab la-stumbleupon-circle',
         'las la-subscript',
         'las la-subway',
         'las la-suitcase',
         'las la-suitcase-rolling',
         'las la-sun',
         'lab la-superpowers',
         'las la-superscript',
         'lab la-supple',
         'las la-surprise',
         'lab la-suse',
         'las la-swatchbook',
         'lab la-swift',
         'las la-swimmer',
         'las la-swimming-pool',
         'lab la-symfony',
         'las la-synagogue',
         'las la-sync',
         'las la-sync-alt',
         'las la-syringe',
         'las la-table',
         'las la-table-tennis',
         'las la-tablet',
         'las la-tablet-alt',
         'las la-tablets',
         'las la-tachometer-alt',
         'las la-tag',
         'las la-tags',
         'las la-tape',
         'las la-tasks',
         'las la-taxi',
         'lab la-teamspeak',
         'las la-teeth',
         'las la-teeth-open',
         'lab la-telegram',
         'lab la-telegram-plane',
         'las la-temperature-high',
         'las la-temperature-low',
         'lab la-tencent-weibo',
         'las la-tenge',
         'las la-terminal',
         'las la-text-height',
         'las la-text-width',
         'las la-th',
         'las la-th-large',
         'las la-th-list',
         'lab la-the-red-yeti',
         'las la-theater-masks',
         'lab la-themeco',
         'lab la-themeisle',
         'las la-thermometer',
         'las la-thermometer-empty',
         'las la-thermometer-full',
         'las la-thermometer-half',
         'las la-thermometer-quarter',
         'las la-thermometer-three-quarters',
         'lab la-think-peaks',
         'las la-thumbs-down',
         'las la-thumbs-up',
         'las la-thumbtack',
         'las la-ticket-alt',
         'las la-times',
         'las la-times-circle',
         'las la-tint',
         'las la-tint-slash',
         'las la-tired',
         'las la-toggle-off',
         'las la-toggle-on',
         'las la-toilet',
         'las la-toilet-paper',
         'las la-toolbox',
         'las la-tools',
         'las la-tooth',
         'las la-torah',
         'las la-torii-gate',
         'las la-tractor',
         'lab la-trade-federation',
         'las la-trademark',
         'las la-traffic-light',
         'las la-train',
         'las la-tram',
         'las la-transgender',
         'las la-transgender-alt',
         'las la-trash',
         'las la-trash-alt',
         'las la-trash-restore',
         'las la-trash-restore-alt',
         'las la-tree',
         'lab la-trello',
         'lab la-tripadvisor',
         'las la-trophy',
         'las la-truck',
         'las la-truck-loading',
         'las la-truck-monster',
         'las la-truck-moving',
         'las la-truck-pickup',
         'las la-tshirt',
         'las la-tty',
         'lab la-tumblr',
         'lab la-tumblr-square',
         'las la-tv',
         'lab la-twitch',
         'lab la-twitter',
         'lab la-twitter-square',
         'lab la-typo3',
         'lab la-uber',
         'lab la-ubuntu',
         'lab la-uikit',
         'lab la-umbraco',
         'las la-umbrella',
         'las la-umbrella-beach',
         'las la-underline',
         'las la-undo',
         'las la-undo-alt',
         'lab la-uniregistry',
         'las la-universal-access',
         'las la-university',
         'las la-unlink',
         'las la-unlock',
         'las la-unlock-alt',
         'lab la-untappd',
         'las la-upload',
         'lab la-ups',
         'lab la-usb',
         'las la-user',
         'las la-user-alt',
         'las la-user-alt-slash',
         'las la-user-astronaut',
         'las la-user-check',
         'las la-user-shield',
         'las la-user-slash',
         'las la-user-tag',
         'las la-user-tie',
         'las la-user-times',
         'las la-users',
         'las la-users-cog',
         'lab la-usps',
         'lab la-ussunnah',
         'las la-utensil-spoon',
         'las la-utensils',
         'lab la-vaadin',
         'las la-vector-square',
         'las la-venus',
         'las la-venus-double',
         'las la-venus-mars',
         'lab la-viacoin',
         'lab la-viadeo',
         'lab la-viadeo-square',
         'las la-vial',
         'las la-vials',
         'lab la-viber',
         'las la-video',
         'las la-video-slash',
         'las la-vihara',
         'lab la-vimeo',
         'lab la-vimeo-square',
         'lab la-vimeo-v',
         'lab la-vine',
         'lab la-vk',
         'lab la-vnv',
         'las la-voicemail',
         'las la-volleyball-ball',
         'las la-volume-down',
         'las la-volume-mute',
         'las la-volume-off',
         'las la-volume-up',
         'las la-vote-yea',
         'las la-vr-cardboard',
         'lab la-vuejs',
         'las la-walking',
         'las la-wallet',
         'las la-warehouse',
         'las la-water',
         'las la-wave-square',
         'lab la-waze',
         'lab la-weebly',
         'lab la-weibo',
         'las la-weight',
         'las la-weight-hanging',
         'lab la-weixin',
         'lab la-whatsapp',
         'lab la-whatsapp-square',
         'las la-wheelchair',
         'lab la-whmcs',
         'las la-wifi',
         'lab la-wikipedia-w',
         'las la-wind',
         'las la-window-close',
         'las la-window-maximize',
         'las la-window-minimize',
         'las la-window-restore',
         'lab la-windows',
         'las la-wine-bottle',
         'las la-wine-glass',
         'las la-wine-glass-alt',
         'lab la-wix',
         'lab la-wizards-of-the-coast',
         'lab la-wolf-pack-battalion',
         'las la-won-sign',
         'lab la-wordpress',
         'lab la-wordpress-simple',
         'lab la-wpbeginner',
         'lab la-wpexplorer',
         'lab la-wpforms',
         'lab la-wpressr',
         'las la-wrench',
         'las la-x-ray',
         'lab la-xbox',
         'lab la-xing',
         'lab la-xing-square',
         'lab la-y-combinator',
         'lab la-yahoo',
         'lab la-yammer',
         'lab la-yandex',
         'lab la-yandex-international',
         'lab la-yarn',
         'lab la-yelp',
         'las la-yen-sign',
         'las la-yin-yang',
         'lab la-yoast',
         'lab la-youtube',
         'lab la-youtube-square',
         'lab la-zhihu',
         'las la-user-circle',
         'las la-user-clock',
         'las la-user-cog',
         'las la-user-edit',
         'las la-user-friends',
         'las la-user-graduate',
         'las la-user-injured',
         'las la-user-lock',
         'las la-user-md',
         'las la-user-minus',
         'las la-user-ninja',
         'las la-user-nurse',
         'las la-user-plus',
         'las la-user-secret',
         'la la-glass',
         'la la-meetup',
         'la la-star-o',
         'la la-star-o',
         'la la-remove',
         'la la-close',
         'la la-gear',
         'la la-trash-o',
         'la la-trash-o',
         'la la-file-o',
         'la la-file-o',
         'la la-clock-o',
         'la la-clock-o',
         'la la-arrow-circle-o-down',
         'la la-arrow-circle-o-down',
         'la la-arrow-circle-o-up',
         'la la-arrow-circle-o-up',
         'la la-play-circle-o',
         'la la-play-circle-o',
         'la la-repeat',
         'la la-rotate-right',
         'la la-refresh',
         'la la-list-alt',
         'la la-dedent',
         'la la-video-camera',
         'la la-picture-o',
         'la la-picture-o',
         'la la-photo',
         'la la-photo',
         'la la-image',
         'la la-image',
         'la la-pencil',
         'la la-map-marker',
         'la la-pencil-square-o',
         'la la-pencil-square-o',
         'la la-share-square-o',
         'la la-share-square-o',
         'la la-check-square-o',
         'la la-check-square-o',
         'la la-arrows',
         'la la-times-circle-o',
         'la la-times-circle-o',
         'la la-check-circle-o',
         'la la-check-circle-o',
         'la la-mail-forward',
         'la la-eye',
         'la la-eye-slash',
         'la la-warning',
         'la la-calendar',
         'la la-arrows-v',
         'la la-arrows-h',
         'la la-bar-chart',
         'la la-bar-chart',
         'la la-bar-chart-o',
         'la la-bar-chart-o',
         'la la-twitter-square',
         'la la-facebook-square',
         'la la-gears',
         'la la-thumbs-o-up',
         'la la-thumbs-o-up',
         'la la-thumbs-o-down',
         'la la-thumbs-o-down',
         'la la-heart-o',
         'la la-heart-o',
         'la la-sign-out',
         'la la-linkedin-square',
         'la la-linkedin-square',
         'la la-thumb-tack',
         'la la-external-link',
         'la la-sign-in',
         'la la-github-square',
         'la la-lemon-o',
         'la la-lemon-o',
         'la la-square-o',
         'la la-square-o',
         'la la-bookmark-o',
         'la la-bookmark-o',
         'la la-twitter',
         'la la-facebook',
         'la la-facebook',
         'la la-facebook-f',
         'la la-facebook-f',
         'la la-github',
         'la la-credit-card',
         'la la-feed',
         'la la-hdd-o',
         'la la-hdd-o',
         'la la-hand-o-right',
         'la la-hand-o-right',
         'la la-hand-o-left',
         'la la-hand-o-left',
         'la la-hand-o-up',
         'la la-hand-o-up',
         'la la-hand-o-down',
         'la la-hand-o-down',
         'la la-arrows-alt',
         'la la-group',
         'la la-chain',
         'la la-scissors',
         'la la-files-o',
         'la la-files-o',
         'la la-floppy-o',
         'la la-floppy-o',
         'la la-navicon',
         'la la-reorder',
         'la la-pinterest',
         'la la-pinterest-square',
         'la la-google-plus-square',
         'la la-google-plus',
         'la la-google-plus',
         'la la-money',
         'la la-money',
         'la la-unsorted',
         'la la-sort-desc',
         'la la-sort-asc',
         'la la-linkedin',
         'la la-linkedin',
         'la la-rotate-left',
         'la la-legal',
         'la la-tachometer',
         'la la-dashboard',
         'la la-comment-o',
         'la la-comment-o',
         'la la-comments-o',
         'la la-comments-o',
         'la la-flash',
         'la la-clipboard',
         'la la-paste',
         'la la-paste',
         'la la-lightbulb-o',
         'la la-lightbulb-o',
         'la la-exchange',
         'la la-cloud-download',
         'la la-cloud-upload',
         'la la-bell-o',
         'la la-bell-o',
         'la la-cutlery',
         'la la-file-text-o',
         'la la-file-text-o',
         'la la-building-o',
         'la la-building-o',
         'la la-hospital-o',
         'la la-hospital-o',
         'la la-tablet',
         'la la-mobile',
         'la la-mobile-phone',
         'la la-circle-o',
         'la la-circle-o',
         'la la-mail-reply',
         'la la-github-alt',
         'la la-folder-o',
         'la la-folder-o',
         'la la-folder-open-o',
         'la la-folder-open-o',
         'la la-smile-o',
         'la la-smile-o',
         'la la-frown-o',
         'la la-frown-o',
         'la la-meh-o',
         'la la-meh-o',
         'la la-keyboard-o',
         'la la-keyboard-o',
         'la la-flag-o',
         'la la-flag-o',
         'la la-mail-reply-all',
         'la la-star-half-o',
         'la la-star-half-o',
         'la la-star-half-empty',
         'la la-star-half-empty',
         'la la-star-half-full',
         'la la-star-half-full',
         'la la-code-fork',
         'la la-chain-broken',
         'la la-shield',
         'la la-calendar-o',
         'la la-calendar-o',
         'la la-maxcdn',
         'la la-html5',
         'la la-css3',
         'la la-ticket',
         'la la-minus-square-o',
         'la la-minus-square-o',
         'la la-level-up',
         'la la-level-down',
         'la la-pencil-square',
         'la la-external-link-square',
         'la la-compass',
         'la la-caret-square-o-down',
         'la la-caret-square-o-down',
         'la la-toggle-down',
         'la la-toggle-down',
         'la la-caret-square-o-up',
         'la la-caret-square-o-up',
         'la la-toggle-up',
         'la la-toggle-up',
         'la la-caret-square-o-right',
         'la la-caret-square-o-right',
         'la la-toggle-right',
         'la la-toggle-right',
         'la la-eur',
         'la la-euro',
         'la la-gbp',
         'la la-usd',
         'la la-dollar',
         'la la-inr',
         'la la-rupee',
         'la la-jpy',
         'la la-cny',
         'la la-rmb',
         'la la-yen',
         'la la-rub',
         'la la-ruble',
         'la la-rouble',
         'la la-krw',
         'la la-won',
         'la la-btc',
         'la la-bitcoin',
         'la la-bitcoin',
         'la la-file-text',
         'la la-sort-alpha-asc',
         'la la-sort-alpha-desc',
         'la la-sort-amount-asc',
         'la la-sort-amount-desc',
         'la la-sort-numeric-asc',
         'la la-sort-numeric-desc',
         'la la-youtube-square',
         'la la-youtube',
         'la la-xing',
         'la la-xing-square',
         'la la-youtube-play',
         'la la-youtube-play',
         'la la-dropbox',
         'la la-stack-overflow',
         'la la-instagram',
         'la la-flickr',
         'la la-adn',
         'la la-bitbucket',
         'la la-bitbucket-square',
         'la la-bitbucket-square',
         'la la-tumblr',
         'la la-tumblr-square',
         'la la-long-arrow-down',
         'la la-long-arrow-up',
         'la la-long-arrow-left',
         'la la-long-arrow-right',
         'la la-apple',
         'la la-windows',
         'la la-android',
         'la la-linux',
         'la la-dribbble',
         'la la-skype',
         'la la-foursquare',
         'la la-trello',
         'la la-gratipay',
         'la la-gittip',
         'la la-gittip',
         'la la-sun-o',
         'la la-sun-o',
         'la la-moon-o',
         'la la-moon-o',
         'la la-vk',
         'la la-weibo',
         'la la-renren',
         'la la-pagelines',
         'la la-stack-exchange',
         'la la-arrow-circle-o-right',
         'la la-arrow-circle-o-right',
         'la la-arrow-circle-o-left',
         'la la-arrow-circle-o-left',
         'la la-caret-square-o-left',
         'la la-caret-square-o-left',
         'la la-toggle-left',
         'la la-toggle-left',
         'la la-dot-circle-o',
         'la la-dot-circle-o',
         'la la-vimeo-square',
         'la la-try',
         'la la-turkish-lira',
         'la la-plus-square-o',
         'la la-plus-square-o',
         'la la-slack',
         'la la-wordpress',
         'la la-openid',
         'la la-institution',
         'la la-bank',
         'la la-mortar-board',
         'la la-yahoo',
         'la la-google',
         'la la-reddit',
         'la la-reddit-square',
         'la la-stumbleupon-circle',
         'la la-stumbleupon',
         'la la-delicious',
         'la la-digg',
         'la la-pied-piper-pp',
         'la la-pied-piper-alt',
         'la la-drupal',
         'la la-joomla',
         'la la-spoon',
         'la la-behance',
         'la la-behance-square',
         'la la-steam',
         'la la-steam-square',
         'la la-automobile',
         'la la-cab',
         'la la-envelope-o',
         'la la-envelope-o',
         'la la-deviantart',
         'la la-soundcloud',
         'la la-file-pdf-o',
         'la la-file-pdf-o',
         'la la-file-word-o',
         'la la-file-word-o',
         'la la-file-excel-o',
         'la la-file-excel-o',
         'la la-file-powerpoint-o',
         'la la-file-powerpoint-o',
         'la la-file-image-o',
         'la la-file-image-o',
         'la la-file-photo-o',
         'la la-file-photo-o',
         'la la-file-picture-o',
         'la la-file-picture-o',
         'la la-file-archive-o',
         'la la-file-archive-o',
         'la la-file-zip-o',
         'la la-file-zip-o',
         'la la-file-audio-o',
         'la la-file-audio-o',
         'la la-file-sound-o',
         'la la-file-sound-o',
         'la la-file-video-o',
         'la la-file-video-o',
         'la la-file-movie-o',
         'la la-file-movie-o',
         'la la-file-code-o',
         'la la-file-code-o',
         'la la-vine',
         'la la-codepen',
         'la la-jsfiddle',
         'la la-life-ring',
         'la la-life-bouy',
         'la la-life-bouy',
         'la la-life-buoy',
         'la la-life-buoy',
         'la la-life-saver',
         'la la-life-saver',
         'la la-support',
         'la la-support',
         'la la-circle-o-notch',
         'la la-rebel',
         'la la-ra',
         'la la-ra',
         'la la-resistance',
         'la la-resistance',
         'la la-empire',
         'la la-ge',
         'la la-ge',
         'la la-git-square',
         'la la-git',
         'la la-hacker-news',
         'la la-y-combinator-square',
         'la la-y-combinator-square',
         'la la-yc-square',
         'la la-yc-square',
         'la la-tencent-weibo',
         'la la-qq',
         'la la-weixin',
         'la la-wechat',
         'la la-wechat',
         'la la-send',
         'la la-paper-plane-o',
         'la la-paper-plane-o',
         'la la-send-o',
         'la la-send-o',
         'la la-circle-thin',
         'la la-circle-thin',
         'la la-header',
         'la la-sliders',
         'la la-futbol-o',
         'la la-futbol-o',
         'la la-soccer-ball-o',
         'la la-soccer-ball-o',
         'la la-slideshare',
         'la la-twitch',
         'la la-yelp',
         'la la-newspaper-o',
         'la la-newspaper-o',
         'la la-paypal',
         'la la-google-wallet',
         'la la-cc-visa',
         'la la-cc-mastercard',
         'la la-cc-discover',
         'la la-cc-amex',
         'la la-cc-paypal',
         'la la-cc-stripe',
         'la la-bell-slash-o',
         'la la-bell-slash-o',
         'la la-trash',
         'la la-copyright',
         'la la-eyedropper',
         'la la-area-chart',
         'la la-pie-chart',
         'la la-line-chart',
         'la la-lastfm',
         'la la-lastfm-square',
         'la la-ioxhost',
         'la la-angellist',
         'la la-cc',
         'la la-cc',
         'la la-ils',
         'la la-shekel',
         'la la-sheqel',
         'la la-meanpath',
         'la la-meanpath',
         'la la-buysellads',
         'la la-connectdevelop',
         'la la-dashcube',
         'la la-forumbee',
         'la la-leanpub',
         'la la-sellsy',
         'la la-shirtsinbulk',
         'la la-simplybuilt',
         'la la-skyatlas',
         'la la-diamond',
         'la la-diamond',
         'la la-intersex',
         'la la-facebook-official',
         'la la-facebook-official',
         'la la-pinterest-p',
         'la la-whatsapp',
         'la la-hotel',
         'la la-viacoin',
         'la la-medium',
         'la la-y-combinator',
         'la la-yc',
         'la la-yc',
         'la la-optin-monster',
         'la la-opencart',
         'la la-expeditedssl',
         'la la-battery-4',
         'la la-battery',
         'la la-battery-3',
         'la la-battery-2',
         'la la-battery-1',
         'la la-battery-0',
         'la la-object-group',
         'la la-object-ungroup',
         'la la-sticky-note-o',
         'la la-sticky-note-o',
         'la la-cc-jcb',
         'la la-cc-diners-club',
         'la la-clone',
         'la la-hourglass-o',
         'la la-hourglass-o',
         'la la-hourglass-1',
         'la la-hourglass-2',
         'la la-hourglass-3',
         'la la-hand-rock-o',
         'la la-hand-rock-o',
         'la la-hand-grab-o',
         'la la-hand-grab-o',
         'la la-hand-paper-o',
         'la la-hand-paper-o',
         'la la-hand-stop-o',
         'la la-hand-stop-o',
         'la la-hand-scissors-o',
         'la la-hand-scissors-o',
         'la la-hand-lizard-o',
         'la la-hand-lizard-o',
         'la la-hand-spock-o',
         'la la-hand-spock-o',
         'la la-hand-pointer-o',
         'la la-hand-pointer-o',
         'la la-hand-peace-o',
         'la la-hand-peace-o',
         'la la-registered',
         'la la-creative-commons',
         'la la-gg',
         'la la-gg-circle',
         'la la-tripadvisor',
         'la la-odnoklassniki',
         'la la-odnoklassniki-square',
         'la la-get-pocket',
         'la la-wikipedia-w',
         'la la-safari',
         'la la-chrome',
         'la la-firefox',
         'la la-opera',
         'la la-internet-explorer',
         'la la-television',
         'la la-contao',
         'la la-500px',
         'la la-amazon',
         'la la-calendar-plus-o',
         'la la-calendar-plus-o',
         'la la-calendar-minus-o',
         'la la-calendar-minus-o',
         'la la-calendar-times-o',
         'la la-calendar-times-o',
         'la la-calendar-check-o',
         'la la-calendar-check-o',
         'la la-map-o',
         'la la-map-o',
         'la la-commenting',
         'la la-commenting-o',
         'la la-commenting-o',
         'la la-houzz',
         'la la-vimeo',
         'la la-vimeo',
         'la la-black-tie',
         'la la-fonticons',
         'la la-reddit-alien',
         'la la-edge',
         'la la-credit-card-alt',
         'la la-codiepie',
         'la la-modx',
         'la la-fort-awesome',
         'la la-usb',
         'la la-product-hunt',
         'la la-mixcloud',
         'la la-scribd',
         'la la-pause-circle-o',
         'la la-pause-circle-o',
         'la la-stop-circle-o',
         'la la-stop-circle-o',
         'la la-bluetooth',
         'la la-bluetooth-b',
         'la la-gitlab',
         'la la-wpbeginner',
         'la la-wpforms',
         'la la-envira',
         'la la-wheelchair-alt',
         'la la-wheelchair-alt',
         'la la-question-circle-o',
         'la la-question-circle-o',
         'la la-volume-control-phone',
         'la la-asl-interpreting',
         'la la-deafness',
         'la la-hard-of-hearing',
         'la la-glide',
         'la la-glide-g',
         'la la-signing',
         'la la-viadeo',
         'la la-viadeo-square',
         'la la-snapchat',
         'la la-snapchat-ghost',
         'la la-snapchat-square',
         'la la-pied-piper',
         'la la-first-order',
         'la la-yoast',
         'la la-themeisle',
         'la la-google-plus-official',
         'la la-google-plus-official',
         'la la-google-plus-circle',
         'la la-google-plus-circle',
         'la la-font-awesome',
         'la la-fa',
         'la la-fa',
         'la la-handshake-o',
         'la la-handshake-o',
         'la la-envelope-open-o',
         'la la-envelope-open-o',
         'la la-linode',
         'la la-address-book-o',
         'la la-address-book-o',
         'la la-vcard',
         'la la-address-card-o',
         'la la-address-card-o',
         'la la-vcard-o',
         'la la-vcard-o',
         'la la-user-circle-o',
         'la la-user-circle-o',
         'la la-user-o',
         'la la-user-o',
         'la la-id-badge',
         'la la-drivers-license',
         'la la-id-card-o',
         'la la-id-card-o',
         'la la-drivers-license-o',
         'la la-drivers-license-o',
         'la la-quora',
         'la la-free-code-camp',
         'la la-telegram',
         'la la-thermometer-4',
         'la la-thermometer',
         'la la-thermometer-3',
         'la la-thermometer-2',
         'la la-thermometer-1',
         'la la-thermometer-0',
         'la la-bathtub',
         'la la-s15',
         'la la-window-maximize',
         'la la-window-restore',
         'la la-times-rectangle',
         'la la-window-close-o',
         'la la-window-close-o',
         'la la-times-rectangle-o',
         'la la-times-rectangle-o',
         'la la-bandcamp',
         'la la-grav',
         'la la-etsy',
         'la la-imdb',
         'la la-ravelry',
         'la la-eercast',
         'la la-eercast',
         'la la-snowflake-o',
         'la la-snowflake-o',
         'la la-superpowers',
         'la la-wpexplorer',
         'la la-spotify'         
       );
       
       $icons['conult-lineawesome-icon'] = array(
           'name'          => 'conult-icons-lineawesome',
           'label'         => esc_html__( 'Line Awesome', 'conult-themer' ),
           'labelIcon'     => 'fas fa-user',
           'prefix'        => '',
           'displayPrefix' => '',
           'icons'         => $lineawesome_icons,
            'url'          => GAVIAS_CONULT_PLUGIN_URL . 'elementor/assets/libs/line-awesome/css/line-awesome.min.css',
           'ver'           => '1.0',
       );

       return $icons;
   }