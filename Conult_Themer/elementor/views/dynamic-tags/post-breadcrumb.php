<?php
   ...
   if(get_post_meta($post_id, 'conult_breadcrumb_layout', true) == 'page_options'){
      $breadcrumb_disable = get_post_meta($post_id, 'conult_no_breadcrumbs', true);
      if($breadcrumb_disable){ return; }

      //Breacrumb Image Color
      $breadcrumb_bg_color = get_post_meta($post_id, 'conult_breacrumb_bg_color', true);
      $breadcrumb_bg_color_opacity = get_post_meta($post_id, 'conult_breacrumb_bg_opacity', true);
      ...
      // Breadcrumb Image
      $breadcrumb_image = get_post_meta($post_id, 'conult_conult_breacrumb_image', true);
      if(is_numeric($breadcrumb_image)){
         $breadcrumb_image_attachment = wp_get_attachment_image_src( $breadcrumb_image, 'full');
         if(isset($breadcrumb_image_attachment[0])){
            $breadcrumb_image_url = $breadcrumb_image_attachment[0];
         }
      }else{
   ...
   if(is_search()){
      $title = esc_html__('Search', 'conult-themer');
   }
